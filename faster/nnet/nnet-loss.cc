...
//<jiayu>
// e.g frame_mask = [1.0, 1.0, 0.0, 1.0],
// the third frame will be masked out, diff is set to zero, and excluded from statistic accumulation
void Xent::EvalMasked(const VectorBase<BaseFloat> &frame_mask_host,
  ...
  // mask
  frame_mask_.Resize(frame_mask_host.Dim());
  frame_mask_.CopyFromVec(frame_mask_host);

  // compute derivative wrt. activations of last layer of neurons
  diff->Resize(num_frames, num_pdf);
  *diff = net_out;
  diff->AddMat(-1.0, tgt_mat_device_);  // y-t
  diff->MulRowsVec(frame_mask_);
  ...
  for(int32 i=0; i<num_frames; i++) {
    if (frame_mask_host(i) == 1) {  // is valid frame
  ...
  xentropy_aux_.ApplyLog(); // log(y)
  xentropy_aux_.MulElements(tgt_mat_device_); // t*log(y)
  xentropy_aux_.MulRowsVec(frame_mask_);
  double cross_entropy = -xentropy_aux_.Sum();
  
  // calculate entropy (in GPU)
  xentropy_aux_ = tgt_mat_device_;   // t
  ...
  entropy_ += entropy;
  correct_ += correct;
  int32 valid_frames = (int32)frame_mask_.Sum();
  frames_ += valid_frames;
  // progressive loss reporting
  {
    //static const int32 progress_step = 3600*100; // 1h
    static const int32 progress_step = 3600*100/10; // 6 min training data
    loss_progress_ += cross_entropy;
    entropy_progress_ += entropy;
    correct_progress_ += correct;
    frames_progress_ += valid_frames;
    if (frames_progress_ > progress_step) {
      KALDI_VLOG(1) << "ProgressLoss[" << (BaseFloat)frames_progress_/100/3600 << "h/" << (BaseFloat)frames_/100/3600 << "h]: " 
                    << (loss_progress_-entropy_progress_)/frames_progress_ << " (Xent)";
      KALDI_VLOG(1) << "FrameAccuracy[" << (BaseFloat)correct_progress_ / frames_progress_ << "]";
      // store
...
void Xent::Eval(const CuMatrixBase<BaseFloat>& net_out, const Posterior& post, CuMatrix<BaseFloat>* diff) {
  ...
  *diff = net_out;
  ...
  // progressive loss reporting
  {
    static const int32 progress_step = 3600*100; // 1h
    //// <jiayu>
    //static const int32 progress_step = 3600*100/30; // 2min
    //// </jiayu>
...
//<jiayu>
void Mse::EvalMasked(const VectorBase<BaseFloat> &frame_mask_host,
                     const CuMatrixBase<BaseFloat>& net_out, 
                     const Posterior& post, 
                     CuMatrix<BaseFloat>* diff) {
  int32 num_frames = net_out.NumRows(),
  ...
  KALDI_ASSERT(net_out.NumCols() == tgt_mat.NumCols());
  KALDI_ASSERT(net_out.NumRows() == tgt_mat.NumRows());
  ...
  //compute derivative w.r.t. neural nerwork outputs
  *diff = net_out; // y
  diff->AddMat(-1.0,CuMatrix<BaseFloat>(tgt_mat)); // (y - t)
  diff->MulRowsVec(frame_mask_); // masking
  ...
  // accumulate
  loss_ += mean_square_error;
  frames_ += frame_mask_host.Sum();

  // progressive loss reporting
  {
    static const int32 progress_step = 1e6; // 2.77h
    frames_progress_ += frame_mask_host.Sum();
    loss_progress_ += mean_square_error;
    if (frames_progress_ > progress_step) {
      KALDI_VLOG(1) << "ProgressLoss[" << frames_progress_/100/3600 << "h/" << frames_/100/3600 << "h]: " 
                    << loss_progress_/frames_progress_ << " (Mse)";
