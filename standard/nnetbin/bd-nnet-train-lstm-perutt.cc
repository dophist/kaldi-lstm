...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "\n"
        "Usage:  bd-nnet-train-lstm-perutt [options] <feature-rspecifier> <targets-rspecifier> <model-in> [<model-out>]\n"
        "e.g.: \n"
        " bd-nnet-train-lstm-perutt scp:feature.scp ark:posterior.ark nnet.init nnet.iter1\n";
    ...
    std::vector<int> new_utt_flags(1, 1);
    ...
    for ( ; !feature_reader.Done(); feature_reader.Next()) {
      ...
      int32 T = mat.NumRows();
      feats_host.Resize(mat.NumRows(), mat.NumCols());
      feats_host.RowRange(0, T-targets_delay).CopyFromMat(mat.RowRange(targets_delay, T-targets_delay));
      for (int t = T-targets_delay; t < T; t++) {
          feats_host.Row(t).CopyFromVec(mat.Row(T-1));
      }
      feats.Resize(feats_host.NumRows(), feats_host.NumCols(), kUndefined);
      feats.CopyFromMat(feats_host);
